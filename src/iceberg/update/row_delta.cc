...
RowDelta& RowDelta::RemoveRows(const std::shared_ptr<DataFile>& file) {
  ICEBERG_BUILDER_CHECK(file != nullptr, "Cannot remove a null data file");
...
Status RowDelta::Validate(const TableMetadata& current_metadata,
  ...
  if (!referenced_data_files_.empty()) {
    ICEBERG_RETURN_UNEXPECTED(MergingSnapshotUpdate::ValidateDataFilesExist(
        ...
        !validate_deletes_, conflict_detection_filter_, snapshot, io, IsCaseSensitive()));
  }
...
Status RowDelta::ValidateNoConflictingFileAndPositionDeletes() const {
  ...
  if (!conflicting_files.empty()) {
    std::string files = "[";
    for (size_t i = 0; i < conflicting_files.size(); ++i) {
      if (i > 0) {
        files += ", ";
      }
      files += conflicting_files[i];
    }
    files += "]";
    return ValidationFailed(
        "Cannot delete data files {} that are referenced by new delete files", files);
  }
